...
function HomePage() {
  const installCmdA = 'npx orchestra-memory@latest init'
  const installCmdB = 'npm install -g orchestra-memory'

  const features = [
    {
      icon: <IconConstellation className="w-10 h-10" />,
      title: 'Cross-project graph',
      body: 'Nodes and typed edges persist across every Claude Code session and project in a local SQLite graph store.',
    },
    {
      icon: <IconHistory className="w-10 h-10" />,
      title: 'Temporal validity',
      body: 'Every memory carries validFrom/validTo windows. Stale facts expire automatically—no manual pruning required.',
    },
    {
      icon: <IconSearch className="w-10 h-10" />,
      title: 'Full-text search',
      body: 'SQLite FTS5 ranks memories by relevance. Claude retrieves exactly the context it needs, nothing more.',
    },
    {
      icon: <IconFlash className="w-10 h-10" />,
      title: 'Auto-injection',
      body: 'A custom slash command injects the top-k relevant memories into every prompt—zero configuration needed.',
    },
    {
      icon: <IconShield className="w-10 h-10" />,
      title: 'Private scoping',
      body: 'Memories scoped to a project ID stay isolated. Promote specific nodes to global scope when needed.',
    },
    {
      icon: <IconDatabase className="w-10 h-10" />,
      title: 'Local-only storage',
      body: 'One SQLite file at ~/.orchestra/memory.db. No embeddings, no API calls, no telemetry. Fully offline.',
    },
  ]
  ...
  const steps = [
    {
      n: '01',
      title: 'Install',
      body: 'Run the init command once. orchestra-memory registers a /memory slash command and creates the local SQLite store.',
    },
    {
      n: '02',
      title: 'Write',
      body: 'Use /memory add to store facts, decisions, team conventions, or anything Claude should recall later.',
    },
    {
      n: '03',
      title: 'Connect',
      body: 'Link memories with typed edges—depends-on, supersedes, related-to—to capture relationships between decisions.',
    },
    {
      n: '04',
      title: 'Query',
      body: 'On every prompt the top-k relevant memories are retrieved via FTS5 and injected automatically into context.',
    },
  ]
  ...
  const faqs = [
    {
      ...
      answer:
        'Anything you tell it to: architectural decisions, recurring bugs, coding conventions, team preferences, external API quirks. Each memory is a node in a SQLite graph with optional metadata, tags, and a validity window.',
    },
    {
      question: 'Where is my data stored?',
      answer:
        'In a SQLite file at ~/.orchestra/memory.db (path is configurable). It never leaves your machine. No sync service, no cloud, no telemetry of any kind.',
    },
    {
      question: 'Does it require an API key or cloud account?',
      answer:
        'No. orchestra-memory is pure local tooling built on the Claude Code slash command API and SQLite. There are no external services or network calls.',
    },
    {
      question: 'How does temporal validity work?',
      answer:
        'Each memory node accepts optional validFrom and validTo ISO timestamps. The retrieval layer automatically filters out expired memories, so sprint goals or deprecated API notes disappear from context when they should.',
    },
    {
      question: 'What is the Orchestra suite?',
      answer:
        'Orchestra is an open-source family of Claude Code plugins built to work together: orchestra-memory (context), orchestra-tasks (task management), orchestra-shell (command history), and orchestra-git (git-aware context). Each can run standalone.',
    },
  ]

  const suite = [
    { name: 'orchestra-memory', desc: 'Cross-project graph memory', active: true },
    { name: 'orchestra-tasks',  desc: 'Task orchestration',         active: false },
    { name: 'orchestra-shell',  desc: 'Shell command history',      active: false },
    { name: 'orchestra-git',    desc: 'Git-aware context',          active: false },
  ]
      ...
      >
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 py-32 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            >
              <span style={{ color: '#c4b5fd' }}>$</span> npx orchestra-memory init
            </a>
            <a
              href="https://github.com/orchestra-suite/orchestra-memory"
              target="_blank"
        ...
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-14">
            ...
            <p className="text-om-text-muted text-sm max-w-md mx-auto">
              A family of open-source Claude Code plugins designed to work together. Use any one standalone,
              or combine them all.
            </p>
          </div>
      ...
      >
        <div className="max-w-3xl mx-auto px-6">
          ...
          >
            ...
            >
              <span className="text-xs font-mono text-om-text-muted">Quickstart (no install)</span>
          ...
          >
            <div
              ...
              style={{ borderColor: 'hsl(220 16% 18%)', backgroundColor: 'hsl(220 16% 9%)' }}
            >
              <span className="text-xs font-mono text-om-text-muted">Global install</span>
            ...
            >
              ...
              {'\n'}
              <span style={{ color: 'hsl(215 12% 50%)' }}>$ </span>
              <span className="text-om-text">orchestra-memory init</span>
            </pre>
          </div>
          ...
          >
            ...
            <p className="text-xs text-om-text-muted leading-relaxed">
              ...
              introduced in Node 22.5 — no extra database driver needed.
            </p>
          </div>
        </div>
      </section>
